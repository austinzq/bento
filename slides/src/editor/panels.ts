...
export class PropsPanel {
  ...
  private buildElementPanel(el: SlideElement) {
    this.section(t({ text: 'Text', shape: 'Shape', image: 'Image', svg: 'Diagram', chart: 'Chart', table: 'Table', media: el.type === 'media' && el.kind === 'audio' ? 'Audio' : 'Video', filter: 'Filter', input: 'Input' }[el.type]))
    this.opsRow([el])
